...
class Cluster {
	...
	public Cluster(final int no, final int l, final int m) {
		isempty = false;
		// this.l = l;
		this.m = m;
		this.no = no;
	...
	public int addobject_mojoplus(final int t, final String object) {
		if (t >= 0 && t < m) {
			if (tags[t] == 0) {
				group = t;
				groupNo += 1;
	...
	public int addobject_mojo(final int t, final String object) {
		if (t >= 0 && t < m) {
			tags[t] += 1;
			totaltags += 1;
	...
	public int addobject(final int t, final String object, final String mode) {
		if (mode.equals("MoJo")) {
			return addobject_mojo(t, object);
		} else {
			return addobject_mojoplus(t, object);
		}
	}
	...
	public String toString() {
		...
		for (int i = 0; i < m; i++) {
			if (objectList.elementAt(i).size() > 0) {
				str = str + "Group " + (i + 1) + ":" + " have "
						+ objectList.elementAt(i).size()
						+ " objects, they are "
						+ objectList.elementAt(i).toString() + "\n";
	...
	public void move(final int grouptag, final Cluster sub) {
		for (int i = 0; i < objectList.elementAt(grouptag).size(); i++) {
			sub.objectList.elementAt(grouptag).addElement(
					objectList.elementAt(grouptag).elementAt(i));
		}
		objectList.elementAt(grouptag).removeAllElements();
	}

	/* merge with another cluster */
	public void merge(final Cluster sub) {
		maxtag += sub.maxtag;
		totaltags += sub.totaltags;
		sub.isempty = true;
		for (int j = 0; j < m; j++) {
			for (int i = 0; i < sub.objectList.elementAt(j).size(); i++) {
				objectList.elementAt(j).addElement(
						sub.objectList.elementAt(j).elementAt(i));
			}
	...
	private boolean isOmniPresent(final String obj, final Vector<?> omniVector) {
		if (omniVector == null || omniVector.size() == 0) {
			return false;
		}
	...
	public double edgeCost(final Hashtable<String, Double> tableR,
			final Cluster[] grouptags, final Vector<?> omniVector) {
		double cost = 0;
			...
			if (j != group) {
				if (grouptags[j] == null) {
					...
					for (int i = 0; i < objectList.elementAt(j).size(); i++) {
						final String obj = objectList.elementAt(j).elementAt(i);
					...
					for (int i = 0; i < objectList.elementAt(j).size(); i++) {
						...
						if (isOmniPresent(obj, omniVector)) {
							 ...
							 * A_i
							 */
							for (int k = 0; k < objectList.elementAt(group)
									.size(); k++) {
								final String obj2 = objectList.elementAt(group)
										.elementAt(k);
								/*
								 ...
								 */
								value1 = tableR.get(obj + "%@$" + obj2) == null ? 0
										: tableR.get(obj + "%@$" + obj2)
												.doubleValue();
								value2 = tableR.get(obj2 + "%@$" + obj) == null ? 0
										: tableR.get(obj2 + "%@$" + obj)
												.doubleValue();
							 ...
							 * A_j
							 */
							for (int l = 0; l < grouptags[j].objectList
									.elementAt(j).size(); l++) {
								final String obj3 = grouptags[j].objectList
										.elementAt(j).elementAt(l);
								value1 = tableR.get(obj + "%@$" + obj3) == null ? 0
										: tableR.get(obj + "%@$" + obj3)
												.doubleValue();
								value2 = tableR.get(obj3 + "%@$" + obj) == null ? 0
										: tableR.get(obj3 + "%@$" + obj)
												.doubleValue();
								edges_target += value1 + value2;
							}
							if (edges_target != edges_source) {
								cost += Math.abs(edges_source - edges_target)
