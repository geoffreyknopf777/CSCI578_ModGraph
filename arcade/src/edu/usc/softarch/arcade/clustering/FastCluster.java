...
public class FastCluster {
	...
	public FastCluster(String name, BitSet featureSet, ArrayList<String> namesInFeatureSet) {
		...
		/*
		 * features = new double[featureLength]; for (int
		 * i=0;i<featureLength;i++) { if (featureSet.get(i)) { features[i] = 1;
		 * } else { features[i] = 0; } }
		 */
	...
	public FastCluster(FastCluster c1, FastCluster c2) {
		/*
		 * double[] features1 = c1.getFeatures(); double[] features2 =
		 * c2.getFeatures(); double[] newFeatures = new
		 * double[features1.length];
		 * 
		 * for (int i=0;i<features1.length;i++) { double featureSum =
		 * features1[i] + features2[i]; double newFeatureVal =
		 * featureSum/(c1.getNumEntities()+c2.getNumEntities()); newFeatures[i]
		 * = newFeatureVal; }
		 */
		 ...
		 * System.arraycopy(newFeatures, 0, this.features, 0,
	...
	public FastCluster(ClusteringAlgorithmType cat, FastCluster c1, FastCluster c2) {
	...
	private void setNonZeroFeatureMapForLibmoUsingIndices(FastCluster c1, FastCluster c2, Set<Integer> c1Indices) {
	...
	private void setNonZeroFeatureMapForWcaUsingIndices(FastCluster c1, FastCluster c2, Set<Integer> c1Indices) {
		for (Integer index : c1Indices) {
			Double c1Value = c1.getNonZeroFeatureMap().get(index);
			Double c2Value = c2.getNonZeroFeatureMap().get(index);
