...
public class SmellEquivalenceTester {
	...
	public void test() {
		...
		Set<Smell> detectedGtSmells = SmellUtil
				.deserializeDetectedSmells("/home/joshua/Documents/Software Engineering Research/projects/recovery/smell_detection/hadoop/ground_truth/hadoop_gt_smells.ser");
		...
		for (Smell smell : detectedGtSmells) {
			...
			if (smell instanceof BuoSmell) {
				...
				/*
				 * for (ConcernCluster cluster : smell.clusters) { if
				 * (cluster.getName().equals("1__Other_utilities")) {
				 * dupes.add(smell); } }
				 */
			}
		}
		 ...
		 * Set<ConcernCluster> dupeClusters2 = dupes.get(1).clusters; for
		 * (ConcernCluster cluster1 : dupeClusters1) { for (ConcernCluster
		 * cluster2 : dupeClusters2) { assertTrue(cluster1.equals(cluster2)); }
		 * }
		 */
